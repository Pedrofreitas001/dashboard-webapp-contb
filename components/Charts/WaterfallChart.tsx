...
const WaterfallChart: React.FC = () => {
  ...
  return (
    <div className="bg-surface-dark border border-border-dark rounded-xl p-6 flex flex-col h-[400px]">
      <h3 className="text-white font-semibold text-lg mb-6">Composição do Resultado (DRE) - Waterfall</h3>
      <div className="flex-1">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={processedData} margin={{ top: 20, right: 0, left: -20, bottom: 20 }}>
            ...
            <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: '#9db9a8', fontSize: 12 }} dy={10} />
            <YAxis axisLine={false} tickLine={false} tick={{ fill: '#9db9a8', fontSize: 12 }} />
            <Tooltip 
               contentStyle={{ backgroundColor: '#1c2720', border: '1px solid #3b5445', borderRadius: '8px' }}
